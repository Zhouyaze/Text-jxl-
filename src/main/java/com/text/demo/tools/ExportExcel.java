...
public class ExportExcel {
     ...
     * @throws Exception
     */
    public static boolean exportData(HttpServletResponse response,String dateformathourminute,String fileName, String[] title, List<String[]> dataList) throws Exception {
    ...
    private static void putExcel(OutputStream os, String fileName, String[] dataTitle, List<String[]> dataList) {
        // 创建工作薄
...
//            cellView.setAutosize(true); // 设置自动大小
            ...
            if (dataTitle.length == 1 && "无数据".equals(dataTitle[0])) {
                sheet.mergeCells(0, 1, dataTitle.length, 0);// 合并单元格
                label = new Label(0, 1, "无数据", titleFormate);
                sheet.addCell(label);
            } else {
                // 设置列表头
                for (int i = 0; i < dataTitle.length; i++) {
                ...
                for (int j = 0; j < dataList.size(); j++) {
                    // 序号
                    number = new jxl.write.Number(0, j + 2, j + 1, titleFormate);
